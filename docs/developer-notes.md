...
4. **Statistic import.** Build external statistics payloads for primary and boost energy (`total_increasing`), runtime (`measurement`), and scheduled time (`measurement`). Persist the cumulative energy totals in the per-entry storage file so repeated runs remain idempotent. When the entity registry does not provide a statistic override, fall back to Home Assistant compatible IDs shaped like `securemtr:securemtr_{entry_slug}_{suffix}`.
